...
#### Make your service independent and call them in main() method
All service should be independent. 
...
Make all service independent and call them in the right order in `main()` method step by step (the result of previous service method should be the input for next one)
...
- `service` for holding model like Fruit (if necessary)
- `service` for holding service, like Writer, Reader, Parser and so on
- `service.impl` for holding implementations of service
